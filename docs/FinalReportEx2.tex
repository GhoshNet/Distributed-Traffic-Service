...
\begin{enumerate}[label=\textbf{FR\arabic*}]
    \item \textbf{User Registration and Authentication:} Drivers and enforcement agents can register accounts and authenticate via JWT tokens. Passwords are hashed with bcrypt. Separate "DRIVER" and "ENFORCEMENT\_AGENT" roles are supported. Vehicle registration is linked to user accounts.
...
\begin{tabularx}{\textwidth}{L{3.2cm} L{4.4cm} X}
  ...
  Booking latency & $<400\,$ms at p95  under normal load & End-to-end saga (journey row $\to$ conflict check $\to$ outbox) benchmarks at 100--250\,ms on the slim stack. \\
...
All external traffic enters through HAProxy, which forwards requests to two nginx instances in full-stack mode or a single nginx instance in slim mode. We implemented an API Gateway pattern using Nginx as the single entry point for all client traffic. This design decouples the frontend from the internal service topology and allows us to enforce global security policies, such as rate limiting (e.g., 5 r/s for auth endpoints). Nginx applies three rate-limit zones: auth (5 r/s, burst 10), booking (10 r/s, burst 20), and general (30 r/s, burst 10–20). It also handles request routing based on URL prefixes. Table~\ref{tab:api} lists all public API endpoints.
...
The User Service provides stateless JWT-based authentication and serves as the entry point for driver and enforcement agent registration. It routes read queries to a Postgres streaming replica and write queries to the primary, implementing read/write separation at the application level by maintaining separate session pools for the primary (writes) and replica (reads), with the appropriate pool selected via dependency injection in each API route.
...
keyed on notif:processed:{MessageId} when the broker did assign a MessageId, which is a SHA-256 hash of the message body is used as fallback when no MessageId is present. On each delivery the consumer first issues "EXISTS" on the key; if present, the message is acknowledged and skipped without invoking the handler. On the non-duplicate path the handler runs and the consumer then calls "SET key 1 EX 86400" to mark the message processed for 24\,hours. This turns the at-least-once delivery guarantee of RabbitMQ into an effectively-exactly-once push to the user. On broker disconnect the consumer reconnects with a fixed 3-second retry interval (up to 10 attempts on initial connect). Unprocessable messages are "Nack"'d without requeue and routed through the shared fanout DLX "journey\_events\_dlx" into the "dead\_letter\_queue"; a 24-hour "x-message-ttl" is additionally set on the main "notification\_events" queue so stuck messages eventually expire into the DLX rather than blocking the consumer.
...
Each node maintains a peer registry (added via "POST /admin/peers/register"). The group is considered healthy when $>$50\% of registered peers are ALIVE; below this threshold the node enters "LOCAL\_ONLY" mode. This is a lightweight threshold quorum, sufficient for a two-laptop demo without full Paxos/Raft complexity.
...
\subsubsection{Consistent-Hash Sharding}
  ...
  \begin{tikzpicture}[>=Stealth,
    box/.style={draw=bordercolor, line width=0.7pt, rounded corners=4pt,
                fill=svcyellow, minimum width=2.6cm, minimum height=0.9cm,
                font=\small, align=center},
    dec/.style={draw=bordercolor, line width=0.7pt, diamond, aspect=2.0,
                fill=svcyellow, font=\scriptsize, align=center,
                minimum width=1.6cm, minimum height=0.9cm}]

    \node[box] (key)  at (0,   0)    {write key\\{\scriptsize(email / route ID)}};
    \node[box] (hash) at (3.4, 0)    {\texttt{MD5(key)}};
    \node[box] (mod)  at (6.4, 0)    {\texttt{mod\;N}};
    \node[dec] (q)    at (9.2, 0)    {shard\\$=0$?};
    \node[box] (pri)  at (12.2, 1.0) {PRIMARY\\write locally};
    \node[box] (fwd)  at (12.2,-1.0) {forward to\\shard-0 node};

    \draw[-{Stealth}] (key)  -- (hash);
    \draw[-{Stealth}] (hash) -- (mod);
    \draw[-{Stealth}] (mod)  -- (q);
    \draw[-{Stealth}] (q.north east) -- node[above, font=\scriptsize]{yes} (pri.west);
    \draw[-{Stealth}] (q.south east) -- node[below, font=\scriptsize]{no}  (fwd.west);

  \end{tikzpicture}
  \caption{Consistent-hash write routing for the User Service (key = email) and
  Conflict Service (key = route ID). $N$ is the number of live nodes.}
  \label{fig:sharding}
  \end{figure}
  
Both User Service and Conflict Service assign write authority via MD5 hash: "shard = MD5(key) \% num\_nodes" where key is the email (users) or route ID (routes). The node where "shard == 0" is PRIMARY. \emph{All nodes store all data} (active-active replication), sharding governs write authority only, so reads can be served from any node (locality). Shard role is logged on every operation and surfaced in the activity feed.
\subsubsection{Client-Side Failover}
  ...
  \begin{tikzpicture}[>=Stealth,
    box/.style={draw=bordercolor, line width=0.7pt, rounded corners=4pt,
                fill=svcyellow, minimum width=3.2cm, minimum height=0.9cm,
                font=\small, align=center},
    dec/.style={draw=bordercolor, line width=0.7pt, diamond, aspect=2.5,
                fill=svcyellow, font=\scriptsize, align=center}]

    \node[box] (req)  at (0,    0)   {API request};
    \node[box] (try)  at (0,  -1.7)  {fetch current URL};
    \node[dec] (ok)   at (0,  -3.3)  {2xx?};
    \node[box] (ret)  at (4.5,-3.3)  {return response};
    \node[dec] (more) at (0,  -4.9)  {peers\\remaining?};
    \node[box] (err)  at (4.5,-4.9)  {return error};
    \node[box] (next) at (0,  -6.5)  {advance to next peer};

    \draw[-{Stealth}] (req)       -- (try);
    \draw[-{Stealth}] (try)       -- (ok);
    \draw[-{Stealth}] (ok.east)   -- node[above, font=\scriptsize]{yes} (ret.west);
    \draw[-{Stealth}] (ok.south)  -- node[right, font=\scriptsize]{no~(5xx / net err)} (more.north);
    \draw[-{Stealth}] (more.east) -- node[above, font=\scriptsize]{no}  (err.west);
    \draw[-{Stealth}] (more.south)-- node[right, font=\scriptsize]{yes} (next.north);
    \draw[-{Stealth}] (next.west) -- ++(-2,0) |- (try.west);

  \end{tikzpicture}
  \caption{\texttt{resilientFetch} client-side failover logic. }
  \label{fig:resilient-fetch}
  \end{figure}


Every browser API call goes through "resilientFetch", which tries the primary URL first, then each peer in the ALIVE list on "5xx" or network error ("4xx" passes through unchanged, a rejected booking is not a node failure). The peer list is fetched from "/health/nodes" at login and persisted to "localStorage", enabling failover even before authentication. The WebSocket to "/ws/notifications/" uses the same list, cycling to a peer after two consecutive disconnects.
...
Testing was conducted at three levels: (1) \textbf{Automated end-to-end}, "scripts/demo\_local.py" runs 11 steps covering health, registration, saga booking, 2PC, conflict detection, cancellation, notifications, enforcement, analytics, outbox durability, and circuit breaker; (2) \textbf{Interactive simulation}, "scripts/simulate\_problems.py" provides 7 CLI scenarios (data conflict, concurrent storm, 2PC demo, failure detection, circuit breaker, graceful degradation, outbox); (3) \textbf{Multi-device}, two laptops on a shared hotspot, registering each other as peers, documented in "docs/MULTI\_LAPTOP\_DEMO.md".
...
\begin{tabularx}{\textwidth}{L{4.8cm} L{1.2cm} X}
  ...
  \midrule
  User register / login / vehicle register & PASS & "demo\_local.py" steps 1--3 \\
  JWT authentication cross-node            & PASS & Laptop A token accepted on Laptop B \\
  Booking saga (single node)               & PASS & "CONFIRMED" within ${\approx}250$\,ms p95 \\
  Idempotent retries                       & PASS & Same "Idempotency-Key" returns cached journey \\
  Conflict detection (single node)         & PASS & Second booking of same slot "REJECTED" \\
  Conflict detection (cross-node)          & PASS & Replication lag ${<}200$\,ms on LAN \\
  Two-phase commit booking                 & PASS & "?mode=2pc" commits atomically \\
  Concurrent booking storm (10 parallel)   & PASS & Exactly 1 "CONFIRMED", 9 "REJECTED" \\
  ...
  Enforcement verification (cache hit)     & PASS & Returns "is\_valid=true" under 10\,ms \\
  Enforcement verification (cache miss)    & PASS & Fallback REST path under 100\,ms \\
  Postgres streaming replication           & PASS & Lag visible at "/api/analytics/replica-lag" \\
  ...
  Client-side failover (login \& all ops)  & PASS & Topbar switches to "Failover: <ip>" \\
  WebSocket failover                       & PASS & Reconnects to peer after 2 closes \\
  Cascade kill (journey + user 503)        & PASS & "/admin/simulate/fail" + cascade \\
  Conflict-service peer failover           & PASS & Local conflict-service stopped; booking routed to peer conflict-service via "conflict\_client.py" \\
...
All 7 simulation scenarios produced expected outcomes. Notable bugs found during testing (full log in "docs/DEBUGGING\_LOG.md"): (1) \emph{schema drift}, "journeys.route\_id" missing in live DB, crashing lifecycle scheduler; fixed with "ALTER TABLE"; (2) \emph{Docker Compose override silently ignored}, "DATABASE\_READ\_URL: " does not clear the base value, causing reads to fail with "gaierror"; fixed by setting it explicitly; (3) \emph{nginx stale upstream IP}, after container recreation, nginx cached old IP returning 404; fixed by switching to per-request DNS resolution ("set \$svc"); (4) \emph{silent cross-node isolation}, two laptops with separate Postgres produced no conflicts, driving the replication design.
